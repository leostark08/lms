            ...
            if (!empty($section->sequence)) {
                ...
                foreach ($sequence as $seq) {
                    ...
                    foreach (array(
                        ...
                        'completionexpected', 'score', 'showdescription', 'deletioninprogress'
                    ) as $property) {
                        if (
                            property_exists($mod[$seq], $property) &&
                    ...
                    if (
                        property_exists($mod[$seq], 'completiongradeitemnumber') &&
...
function course_delete_module($cmid, $async = false)
{
    ...
    if ($grade_items = grade_item::fetch_all(array(
        'itemtype' => 'mod', 'itemmodule' => $modulename,
    ...
    $DB->delete_records('course_completion_criteria', array(
...
function move_section_to($course, $section, $destination, $ignorenumsections = false)
{
    ...
    if (!$sections = $DB->get_records_menu(
        'course_sections',
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null)
{
    ...
    if (!isset($str)) {
        $str = get_strings(array(
            'delete', 'move', 'moveright', 'moveleft',
            'editsettings', 'duplicate', 'modhide', 'makeavailable', 'makeunavailable', 'modshow'
        ), 'moodle');
    ...
    if ($hasmanageactivities && $indent >= 0) {
        ...
        $actions['moveright'] = new action_menu_link_secondary(
            ...
            $str->moveright,
            array(
                'class' => 'editing_moveright ' . $enabledclass, 'data-action' => 'moveright',
        ...
        $actions['moveleft'] = new action_menu_link_secondary(
            ...
            $str->moveleft,
            array(
                'class' => 'editing_moveleft ' . $enabledclass, 'data-action' => 'moveleft',
                'data-keepopen' => true, 'data-sectionreturn' => $sr
            )
        );
    }
    ...
    if ($hasmanageactivities && !$mod->coursegroupmodeforce) {
        if (plugin_supports('mod', $mod->modname, FEATURE_GROUPS, false)) {
            ...
            $actions[$actionname] = new action_menu_link_primary(
                ...
                $grouptitle,
                array(
                    'class' => 'editing_' . $actionname, 'data-action' => $nextactionname,
...
function include_course_ajax($course, $usedmodules = array(), $enabledmodules = null, $config = null)
{
    ...
    if (course_format_uses_sections($course->format)) {
        $PAGE->requires->yui_module(
            ...
            'M.course.init_section_dragdrop',
            array(array(
                'courseid' => $course->id,
        ...
        $PAGE->requires->yui_module(
            'moodle-course-dragdrop',
            'M.course.init_resource_dragdrop',
            array(array(
                'courseid' => $course->id,
...
function get_sorted_course_formats($enabledonly = false)
{
    ...
    if (!empty($CFG->format_plugins_sortorder)) {
        ...
        $order = array_merge(
            array_intersect($order, array_keys($formats)),
...
function mod_duplicate_activity($course, $cm, $sr = null)
{
    ...
    if ($newcm) {
        ...
        $modulehtml = $courserenderer->course_section_cm(
            $course,
            $completioninfo,
            $newcm,
            null,
 ...
 * @return \core_tag\output\tagindex
 */
function course_get_tagged_course_modules(
    $tag,
    $exclusivemode = false,
    $fromcontextid = 0,
    $contextid = 0,
    ...
    $params = array(
        'itemtype' => 'course_modules', 'tagid' => $tag->id, 'component' => 'core',
...
function course_validate_dates($coursedata)
{
    ...
    if (
        !empty($coursedata['startdate']) && !empty($coursedata['enddate']) &&
...
function course_filter_courses_by_timeline_classification(
    ...
    foreach ($courses as $course) {
        ...
        if (
            $classification == COURSE_TIMELINE_ALLINCLUDINGHIDDEN || ($classification == COURSE_TIMELINE_HIDDEN && $pref) ||
...
function course_get_recent_courses(int $userid = null, int $limit = 0, int $offset = 0, string $sort = null)
{
    ...
    $params = [
        'userid' => $userid, 'contextlevel' => CONTEXT_COURSE, 'visible' => 1, 'status' => ENROL_USER_ACTIVE,
        'statusenrol' => ENROL_INSTANCE_ENABLED, 'guestenrol' => 'guest', 'now1' => $now, 'now2' => $now,
        'userid2' => $userid, 'statusenrolg' => ENROL_INSTANCE_ENABLED
    ] + $favparams;
